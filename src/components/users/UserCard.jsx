...
export default function UserCard({ user, role, trust, sangh, status, onView, index = 0 }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-[16px] font-bold text-slate-800 truncate group-hover:text-teal-700">{user.name}</h3>
            <p className="text-[12px] font-semibold text-teal-700 truncate mt-1">{role?.name || 'No role assigned'}</p>
          </div>
        </div>
      ...
      <p className="mt-4 min-h-[40px] text-[13px] leading-5 text-slate-500">
        {user.notes || 'User profile connected with the current trust and sangh structure.'}
      </p>
    </div>
  )
}
