...
export const PERM_GROUPS = [
  ...
  {
    key: "trust",
    label: "Trust Create / Edit / Delete",
    icon: "Landmark",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Trust",
  },
  {
    key: "sangh",
    label: "Sangh Create / Edit / Delete",
    icon: "Users",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
  {
    key: "location",
    label: "Location Create / Edit / Delete",
    icon: "MapPin",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "System",
  },
  {
    key: "accounts",
    label: "Accounts Create / Edit / Delete",
    icon: "Receipt",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Trust",
  },
  {
    key: "committee",
    label: "Committee Create / Edit / Delete",
    icon: "Users",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
  {
    key: "derasar",
    label: "Derasar Create / Edit / Delete",
    icon: "Gem",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
  {
    key: "pathshala_students",
    label: "Pathshala Student Create / Edit / Delete",
    icon: "BookOpen",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
  {
    key: "pathshala_exams",
    label: "Pathshala Exam Create / Edit / Delete",
    icon: "BookOpen",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
  {
    key: "pathshala_results",
    label: "Pathshala Result Create / Edit / Delete",
    icon: "BarChart3",
    perms: ["View", "Create", "Edit", "Delete"],
    type: "Sangh",
  },
...
// Internal map used to quickly resolve keys to human-readable labels
let PERM_LABEL_MAP = {};
function rebuildLabelMap() {
  ...
  PERM_GROUPS.forEach(function (group) {
    group.perms.forEach(function (permission) {
      PERM_LABEL_MAP[
        group.key + "_" + permission.toLowerCase().replace(/\s+/g, "_")
      ] = permission;
    });
  });
}
...
// Helper to get total number of discrete permissions available
export function getTotalPerms() {
  return PERM_GROUPS.reduce(function (sum, group) {
    return sum + group.perms.length;
  }, 0);
}

// Helper to get all permission keys (e.g., "users_create")
...
// Filter permission groups based on hierarchy type
export function getPermsByType(type) {
  return PERM_GROUPS.filter(function (group) {
    return group.type === type;
  });
}

// Get all permission keys for a specific hierarchy type
...
// Resolve a specific role by its ID
export function getRoleById(id) {
  return INITIAL_ROLES.find(function (role) {
    return role.id === Number(id);
  });
}

// Utility to generate a permission key string
export function P(groupKey, action) {
  return groupKey + "_" + action.toLowerCase().replace(/\s+/g, "_");
}

// Resolves actual permission keys for a role, handling 'all' alias
...
// Check if a specific permission key is granted to a role
export function hasPerm(role, key) {
  if (!role) return false;
  if (role.permissions === "all") return true;
  return (
    Array.isArray(role.permissions) && role.permissions.indexOf(key) !== -1
  );
}
...
// Converts flat keys array into a nested object structure for the Form UI
  ...
  perms.forEach(function (key) {
    ...
    const group = PERM_GROUPS.find(function (item) {
      return item.perms.some(function (permission) {
        return (
          item.key + "_" + permission.toLowerCase().replace(/\s+/g, "_") === key
        );
      });
    });
...
// Converts nested UI object back into flat keys for storage
...
export function addPermGroup(key, label, icon, type) {
  if (
    PERM_GROUPS.find(function (group) {
...
export function deletePermGroup(groupKey) {
  const index = PERM_GROUPS.findIndex(function (group) {
...
export function sortRoles(roles, sortBy, sortDir, users) {
  return roles.slice().sort(function (a, b) {
    ...
    if (sortBy === "name") comparison = a.name.localeCompare(b.name);
      ...
      comparison = getUserCount(a.id, users) - getUserCount(b.id, users);
...
export const INITIAL_ROLES = [
  {
    ...
    type: "System",
    description:
      "Complete access across all trusts, sanghs, modules, and permission controls.",
    status: "Active",
    isLocked: true,
    permissions: "all",
  },
  ...
  },
  {
    ...
    type: "Sangh",
    description: "Operational management of specific Sangh departments; can add and edit records but cannot delete.",
    status: "Active", 
    isLocked: false,
    permissions: [
      P("dashboard", "View"),
  ...
  },
  {
    ...
    name: "Staff",
    type: "Trust",
    description: "Handles day-to-day accounts entry and financial report viewing.",
    status: "Active",
    isLocked: false,
    permissions: [
      P("dashboard", "View"),
  ...
  },
  {
    ...
    type: "System",
    description:
      "View-only access across the assigned trust and sangh records.",
    status: "Active",
    isLocked: false,
    permissions: [
      P("dashboard", "View"),
...
// Master list of initial users for demonstration and stats
