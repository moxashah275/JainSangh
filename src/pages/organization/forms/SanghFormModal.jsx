...
export default function SanghFormModal({ isOpen, onClose, initialData, onSave }) {
  const [formData, setFormData] = useState({ name: '', city: '', members: 0, address: '', type: 'Main', status: true });

  useEffect(() => {
    if (initialData) setFormData(initialData);
    else setFormData({ name: '', city: '', members: 0, address: '', type: 'Main', status: true });
  }, [initialData, isOpen]);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="p-6 space-y-4">
          <div>
            <label className="block text-[13px] font-bold text-slate-700 mb-1.5">Sangh Name</label>
            <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all" placeholder="Enter sangh name" />
          </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" className="px-5 py-2 text-[13px] font-bold text-white bg-teal-600 hover:bg-teal-700 rounded-xl shadow-sm shadow-teal-200 transition-all">Create Sangh</button>
          </div>
        </form>
      </div>
    </div>
