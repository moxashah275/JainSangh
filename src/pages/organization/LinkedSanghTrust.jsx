...
export default function LinkedSanghTrust({ onDataChange }) {
  ...
  const handleRemoveLink = (id) => {
    ...
    saveOrgData({ ...data, links: newLinks });
  ...
  return (
    <div>
      {toast.show && (
        <div className="fixed top-8 right-8 z-[999] bg-emerald-500 text-white px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-bounce">
          <Check size={20} /> <span className="font-bold uppercase text-xs">{toast.message}</span>
        </div>
      )}

      {/* Top Section: Link Trigger */}
      <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-white border border-emerald-500 text-emerald-600 hover:bg-emerald-50 px-4 py-2 rounded-xl font-bold text-sm transition-all shadow-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {data.links.length > 0 ? data.links.map(link => (
              <tr key={link.id} className="hover:bg-slate-50/50">
                <td className="px-6 py-4 text-sm font-bold text-slate-700">{getTrustName(link.trustId, data.trusts)}</td>
                <td className="px-6 py-4 text-sm font-semibold text-slate-600">{getSanghName(link.sanghId, data.sanghs)}</td>
                <td className="px-6 py-4 text-center">
                  <span className="px-2 py-1 bg-emerald-100 text-emerald-600 rounded text-[10px] font-bold uppercase">Active</span>
                </td>
                <td className="px-6 py-4 text-center">
                  <button onClick={() => handleRemoveLink(link.id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Remove Link">
                    <Unlink size={16} />
                  </button>
                </td>
              </tr>
            )) : (
              <tr><td colSpan="4" className="py-8 text-center text-sm text-slate-400">No links found. Click "Create New Link" to start.</td></tr>
            )}
          </tbody>
        </table>
      </div>
